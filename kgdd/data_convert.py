...
def write_entities(entity2id, entity_names, parent):
    ...
    with open(ENTITIES_PATH, "w", encoding="utf-8") as fout:
        for node_id, raw_idx in entity2id.items():
            root = find_root(parent, raw_idx)
...
def stream_edges(edges_file, entity2id, publication_entity_ids):
    ...
    with open(edges_file, "r", encoding="utf-8") as fin, open(EDGES_BIN_PATH, "wb") as fout:
        for line in fin:
            edge = json.loads(line)
...
def build_and_write_subclass_graph(
    ...
    category2id = {}
    with open(ENTITIES_PATH, "a", encoding="utf-8") as fout:
    ...
    with open(SUBCLASS_EDGE_LIST_PATH, "w", encoding="utf-8") as fout:
...
def write_relation_hierarchy_edge_list(relation2id, toolkit):
    ...
    with open(RELATION_HIERARCHY_EDGE_LIST_PATH, "w", encoding="utf-8") as fout:
